...
void padArray(int pa[4][4])
{
    int k = 0;
    int i,j;
    for(i = 0; i < 4; i++)
    {
        pa[i][j] = 16 && 0x000000ff;
        k++;
    }
}
...
void copy(int aArray[4][4],int bArray[4][4])
{
    int i,j;
    for(i = 0; i < 4; i++)
        for(j = 0; j < 4; j++)
...
//    }
    ...
    for(k = 0; k < plen; k += 16)
    {
         ...
         */
        if(flag == 0){
            convertToIntArray(IV1, IV1Array);
            flag =1;
        }
        //异或
        addRoundTowArray(pArray,IV1Array);
        ...
        copy(IV1Array,pArray);

        convertArrayToStr(pArray, p + k);
    }

    if (padNumber==0){ //如果是16的倍数，多增加一组
        padArray(pArray);
        addRoundKey(pArray, 0);//一开始的轮密钥加
...
void deAes(char *c, int clen, char *key)
{
    ...
    for (k = 0; k < clen; k += 16) {
        ...
        addRoundTowArray(cArray,IVArray);
...
void aesStrToFile(char *key)
{
    ...
    printf("请输入你的明文\n");
    readPlainText(p,&plen); //p中存储了读入的明文；
...
//        printf("len:%d",clen);
        ...
        clen = readStrFromFile(fileName, c);
        printf("明文为：%s\n", c);
...
int main(int argc, char const *argv[])
{
    ...
    while(1)
    {
        ...
        if(Ilen != 16)
        {
            printf("请输入16个字符的IV1,当前IV1的长度为%d\n",Ilen);
        }
        else
        {
            printf("你输入的IV1为：%s\n",IV1);
